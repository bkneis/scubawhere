...
class CrmCampaignService {
	...
	public function get($id) {
		return $this->crm_campaign_repo->get($id, ['tokens', 'groups', 'crmLinks']);
	}
	...
	public function getAll() {
		return $this->crm_campaign_repo->all(['tokens', 'groups', 'crmLinks']);
	}
	...
	public function getAllWithTrashed() {
		return $this->crm_campaign_repo->allWithTrashed(['tokens', 'groups', 'crmLinks']);
	}
	 ...
	 * @return \Scubawhere\Entities\CrmCampaign
	 */
	public function create(array $data, array $group_ids, $customer_id)
	{
		/**
        ...
        if (count($group_ids) < 1 && (int) $data['sendallcustomers'] !== 1 && $data['is_campaign'] !== 0) {
        	throw new HttpNotAcceptable(__CLASS__.__METHOD__, ['Please specify atleast one group to send the email too']);
        }
