...
class FixAndReseedCountriesAndCurrenciesTables extends Migration {
	...
	public function up()
	{
		...
		foreach($companies as $company)
		{
			if(array_key_exists($oldCurrencies[$company->currency_id], $newCurrencies))
				...
				echo 'Could not reassign currency '.$company->currency_id.' => '.$oldCurrencies[$company->currency_id]."\n";

			if(array_key_exists(ucwords(strtolower($oldCountries[$company->country_id])), $newCountries))
				$company->country_id  = $newCountries[ ucwords(strtolower($oldCountries[$company->country_id])) ];
			else
				echo 'Could not reassign country '.$company->country_id.' => '.$oldCountries[$company->country_id]."\n";
		...
		foreach($customers as $customer)
		{
			...
			if(array_key_exists(ucwords(strtolower($oldCountries[$customer->country_id])), $newCountries))
				$customer->country_id = $newCountries[ ucwords(strtolower($oldCountries[$customer->country_id])) ];
			else
				echo 'Could not reassign country '.$customer->country_id.' => '.$oldCountries[$customer->country_id]."\n";
		...
		foreach($payments as $payment)
		{
			if(array_key_exists($oldCurrencies[$payment->currency_id], $newCurrencies))
				...
				echo 'Could not reassign currency '.$payment->currency_id.' => '.$oldCurrencies[$payment->currency_id]."\n";
		...
		foreach($refunds as $refund)
		{
			if(array_key_exists($oldCurrencies[$refund->currency_id], $newCurrencies))
				...
				echo 'Could not reassign currency '.$refund->currency_id.' => '.$oldCurrencies[$refund->currency_id]."\n";
