...
class ModelTestHelper{
	...
	const TEST_AGENT_REFERENCE         = 'randomString';
	const TEST_AGENT_REFERENCE_UPDATED = 'randomString 2';
	const TEST_BOOKING_STATUS          = "on hold";
	const TEST_BOOKING_STATUS_UPDATED  = "confirmed";
	const TEST_BOOL                    = 0;
	...
	public static function createBooking($company_id, $agent_id, $append = ""){
		...
		$entry->agent_reference  = self::TEST_AGENT_REFERENCE;
		$entry->price            = self::TEST_PRICE;
		$entry->discount         = self::TEST_INTEGER;
		$entry->status           = self::TEST_BOOKING_STATUS;
