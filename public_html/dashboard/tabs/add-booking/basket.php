											...
											<div class="panel-body">
												{{#each addons}}
													<p>
														...
														{{#unless compulsory}}{{#unless pivot.packagefacade_id}}<a class="remove-addon pull-right" href="javascript:void(0);" title="Remove Addon" data-id="{{id}}" data-bookingdetail-id="{{../../id}}"><i class="fa fa-times fa-lg"></i></a>{{else}} <small>(compulsory)</small>{{/unless}}{{/unless}}
													</p>
												{{/each}}
											</div>
										</div>
