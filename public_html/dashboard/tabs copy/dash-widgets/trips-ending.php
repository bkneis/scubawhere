	...
	foreach($allTrips as $trip){
		...
		if(is_array($tripSessions)){
			foreach($tripSessions as $session){
				...
				if(strpos($session['end'], $today) !== false){ 
					$startingTrips[] = array(
						'sessionID'  => $session['id'],
						'tripID'  => $trip['id'],
						'time'  => date("g:ia", strtotime($session['end'])),
						'name'  => $trip['name'],
...
<div class="block-title">Trips Ending Today</div>
...
<?php }else{ ?>
	<div class="dash-trip-mssg">
			<?php echo "There are no trips ending today.."; ?>
	</div>
<?php } ?>
